...
function Article() {
    ...
    useEffect(() => {
        fetch(`https://chi-sat.onrender.com/post/${id}`)
            .then(res => {
    ...
    const deletePost = async () => {
        const response = await fetch(`https://chi-sat.onrender.com/delete-post/${id}`, {
            method: "DELETE",
        })
        if (response.ok) {
            setRedirect(true);
  ...
  return (
    <div className='container pt-5 pb-5'>
        ...
        <div className='article__img'>
            <img src= {`https://chi-sat.onrender.com/${postInfo.cover}`} alt="" />
        </div>
